(() => {
  ...
  let query='',filter='all',favorites=[],notices=[];
  ...
  function matches(f){return (!query||`${f.name} ${f.category}`.toLowerCase().includes(query))&&(filter==='all'||filter==='saved'&&favorites.includes(f.id)||filter==='veg'&&((f.diet||'vegetarian')==='vegetarian'||f.options?.every(o=>o.diet==='vegetarian'))||filter==='egg'&&(f.diet==='egg'||f.diet==='nonvegetarian'||f.options?.some(o=>o.diet==='egg'||o.diet==='nonvegetarian')));}
  ...
  function card(f,featured=false){const idx=visible.push(f)-1,q=itemQuantity(f);return `<article class="food-card ${featured?'featured-card':''} ${photo(f)?'has-photo':'icon-card'}" data-category="${escape(f.category)}"><button class="food-open" data-item="${idx}" aria-label="View ${escape(f.name)} details"><span class="food-photo${photoClass(f)}">${photoMarkup(f)}</span><span class="food-info">${featured?`<span class="feature-kicker">${escape(meal)} highlight</span>`:''}<span class="food-title">${escape(f.name)}${status(f)}</span><span class="food-meta">${f.options?'<span class="choice-label">Choice of options</span>':''}${f.issue&&!flagship?'<span class="rotation-note">To confirm</span>':''}</span>${tracking()&&f.sourceQuantity?`<span class="food-source-quantity">Menu reference: ${escape(f.sourceQuantity)}</span>`:''}</span></button>${flagship?`<button class="save-dish" data-favorite="${escape(f.id)}" aria-label="${favorites.includes(f.id)?'Unsave':'Save'} ${escape(f.name)}" aria-pressed="${favorites.includes(f.id)}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.7l-1.1-1.1a5.5 5.5 0 0 0-7.8 7.8L12 21l8.8-8.6a5.5 5.5 0 0 0 0-7.8Z"/></svg></button>`:''}${tracking()?`<div class="card-measure"><p>${q?`${fmt(q)} on your plate`:'Add what you ate'}</p><button class="add-button ${q?'added':''}" data-item="${idx}" aria-label="Add ${escape(f.name)} to plate">${q?'Edit':flagship?'+ Add':'+'}</button></div>`:''}</article>`;}
  function renderMenu(reason='menu'){if(!data)return;$('menu-sections').dataset.transition=reason;rotation=window.RuchiRotation.weekFor(date,data.rotationRule);visible=[];const weekly=data.days[days[date.getDay()]][meal].filter(allowed),ids=new Set(weekly.map(f=>f.id));const common=data.common[meal].filter(f=>!ids.has(f.id));$('menu-title').textContent=meal;$('schedule-note').hidden=!(rotation===null&&data.days[days[date.getDay()]][meal].some(f=>f.weeks))&&!data.days[days[date.getDay()]][meal].some(f=>f.issue==='wed-fruit');
    const all=[...weekly,...common],filtered=all.filter(matches);
    let groups=[['On the menu',weekly.filter(matches)],['Every day',common.filter(matches)]].filter(([,a])=>a.length);
    if(flagship){
      $('menu-count').textContent=`${filtered.length} ${filtered.length===1?'dish':'dishes'}`;
      $('search-empty').hidden=!!filtered.length;
      $('search-empty').textContent=filter==='saved'?'No saved dishes in this meal yet. Tap a heart to save one.':'No dishes in this category for this meal.';
    ...
    }$('menu-sections').innerHTML=groups.map(([label,items])=>`<section aria-label="${label}"><h3 class="group-heading">${label}</h3><div class="food-grid">${items.map(f=>card(f)).join('')}</div></section>`).join('');
    document.querySelectorAll('[data-meal]').forEach(b=>{const on=b.dataset.meal===meal;b.classList.toggle('active',on);b.setAttribute('aria-pressed',on);});renderTotals();}
  ...
  document.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;if(b.dataset.filter){filter=b.dataset.filter;renderMenu();}
    ...
    if(b.dataset.dock){const action=b.dataset.dock;if(action==='menu'){filter='all';query='';if($('dish-search'))$('dish-search').value='';renderMenu();window.scrollTo({top:0,behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'});}if(action==='search'&&$('dish-search')){$('dish-search').scrollIntoView({block:'center',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'});$('dish-search').focus({preventScroll:true});}if(action==='saved'){filter='saved';query='';if($('dish-search'))$('dish-search').value='';renderMenu();$('menu').scrollIntoView({block:'start',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'});}if(action==='notes'){$('menu-info').open=true;$('menu-info').scrollIntoView({block:'center',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'});}}
