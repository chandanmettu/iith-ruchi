(() => {
  ...
  function renderMenu(reason='menu'){if(!data)return;$('menu-sections').dataset.transition=reason;rotation=window.RuchiRotation.weekFor(date,data.rotationRule);visible=[];
    ...
    const all=saved?[...weekly,...common,...paid]:isExtras?paid:[...weekly,...common],filtered=all.filter(matches);
    let groups=(saved?[['Regular menu',[...weekly,...common].filter(matches)],['Paid extras',scheduled.filter(matches)],['Rotating Saturday extras',rotating.filter(matches)]]:isExtras?[['At the extras counter',scheduled.filter(matches)],['Rotating Saturday extras',rotating.filter(matches)]]:[['On the menu',weekly.filter(matches)],['Every day',common.filter(matches)]]).filter(([,a])=>a.length);
    if($('menu-kind')){
      ...
      $('extras-context').hidden=!isExtras;
      $('extras-context-text').textContent=extras?(rotating.length?'Saturday chaat rotates. These are the listed choices and prices; check the counter for this week’s selection.':`Paid separately at the counter · ${days[date.getDay()]} ${meal.toLowerCase()}. Prices follow the posted menu; availability may vary.`):'The extras list could not load. Reload to try again; the regular menu is still available.';
    }
    if(flagship){
      ...
      $('search-empty').textContent=saved?'No saved dishes in this meal yet. Tap a heart to save one.':isExtras&&!extras?'Extras are temporarily unavailable. Please reload.':isExtras&&!paid.length?'No paid extras are listed for this meal. The regular menu is still available.':'No dishes in this category for this meal.';
